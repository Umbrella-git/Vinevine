...
import logo1 from "@/assets/img/logo/V.svg";
import logo2 from "@/assets/img/logo/VINEVINE.svg";
...
const currentImage = computed(() => {
  if (currentIndex.value > 0) {
    return new URL(logo1, import.meta.url).href;
  } else {
    return new URL(logo2, import.meta.url).href;
  }
});
...
const handleScroll = () => {
  ...
  currentIndex.value = scrollTop / (scrollHeight - clientHeight);
...
.fade-enter-active,
.fade-leave-active {
  transition: opacity 1s; /* 设置动画时间为1秒 */
}
.fade-enter-from,
.fade-leave-to {
