...
const logoM = import.meta.glob("@/assets/img/logo/**/*.{png,jpg,jpeg,gif}", {
  as: "url",
}); // 图片数组
const currentIndex = ref(0); // 当前图片索引
const scrollContainer = ref(null);
const logo = ref<string[]>([]);
for (const key in logoM) {
  obj[key]().then((res) => {
    logo.value.push(res);
  });
}
...
const currentImage = computed(() => {
  return logo.value[currentIndex.value];
});
const logoUrl = new URL("@/assets/img/logo/V.svg", import.meta.url).href;
console.log(logoUrl);
// 处理滚动事件
const handleScroll = () => {
